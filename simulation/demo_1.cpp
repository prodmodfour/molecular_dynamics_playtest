...
int main(int argc, char *argv[])
{
    while (true)
    {
        ...
        PlaybackManager playback_manager = PlaybackManager(total_timesteps);
        ...
        MainWindow main_window = MainWindow(simulation_data, playback_manager);
        main_window.show();

        int exitCode = app.exec();
        // If exitCode == 1, we interpret it as "restart"
        if (exitCode == 1)
        {
            // Loop again, creating a new QApplication and MainWindow
            simulator_thread.join();
            continue;
        }
        else
        {
            // Any other code means normal exit, break out of the loop
            break;
        }
    }
